...
train_imgs_dir = path.join(cityscapes_dir, "leftImg8bit/train/")
train_gt_dir = path.join(cityscapes_dir, "gtFine/train/")

val_imgs_dir = path.join(cityscapes_dir, "leftImg8bit/val/")
val_gt_dir = path.join(cityscapes_dir, "gtFine/val/")
...
for dir_step, dir in enumerate(train_dirs):
    ...
    for step, file_name in enumerate(file_names):
        ...
        if not saved:
            raise IOError(f'cv2 failed to save the file. Make sure that the output directory exist. {img_small_path}')
...
# # save to disk:
cPickle.dump(mean_channels, open(path.join(output_dir, "mean_channels.pkl"), "wb"))
...
# # save to disk:
cPickle.dump(class_weights, open(path.join(output_dir, "class_weights.pkl"), "wb"))
...
for dir_step, dir in enumerate(val_dirs):
    ...
    for step, file_name in enumerate(file_names):
        ...
        # save the label image to output_dir
        trainId_label_path = path.join(output_dir, img_id + "_trainId_label.png")
...
cPickle.dump(val_trainId_label_paths,
             open(path.join(output_dir, "val_trainId_label_paths.pkl"), "wb"))
...
for step, (img_path, label_path) in enumerate(zip(train_img_paths, train_trainId_label_paths)):
    ...
    # flip the image and save to output_dir:
    ...
    # flip the label image and save to output_dir:
...
cPickle.dump(train_img_paths,
             open(path.join(output_dir, "train_img_paths.pkl"), "wb"))
cPickle.dump(train_trainId_label_paths,
             open(path.join(output_dir, "train_trainId_label_paths.pkl"), "wb"))
