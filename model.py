...
from config import run_dir, output_dir
...
class ENet_model(object):
    ...
    def __init__(self, model_id, img_height=512, img_width=1024, batch_size=4):
        ...
        self.logs_dir = path.join(self.run_dir, "logs/")
        ...
        weight_path = path.join(output_dir, "class_weights.pkl")
        self.class_weights = cPickle.load(open(weight_path, "rb"))
    ...
    def create_model_dirs(self):
        self.model_dir = path.join(self.logs_dir, "model_%s" % self.model_id + "/")
        self.checkpoints_dir = path.join(self.model_dir, "checkpoints/")
        self.debug_imgs_dir = path.join(self.model_dir, "imgs/")
